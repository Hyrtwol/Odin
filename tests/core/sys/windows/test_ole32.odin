...
string_from_clsid :: proc(t: ^testing.T) {
	...
	p : LPOLESTR
	hr := StringFromCLSID(CLSID_FileOpenDialog, &p)
	defer if p != nil {CoTaskMemFree(p)}

	testing.expectf(t, SUCCEEDED(hr), "%x (should be: %x)", u32(hr), 0)
	testing.expectf(t, p != nil, "%v is nil", p)

	str, err := wstring_to_utf8(p, 40)
